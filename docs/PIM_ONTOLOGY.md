...
#### What remains PROPOSED (not yet implemented)

| Aspect | Status | Design Intent |
|--------|--------|--------------|
| **ClassificationRule.cluster_id** | Not implemented | Back-reference on the Rule itself (currently the reverse: `TransactionCluster.rule_id`). Would enable explicit rule → cluster ownership |
| **`merging` status on TransactionCluster** | Not implemented | Persistent clusters have no status field. The merging review concept exists only as ProposalCluster status |
| **Merging review for rule-matched txns** | Not implemented | Currently rules auto-assign `txn.cluster_id` immediately. Proposed: both rule-matched and embedding-detected txns go through a merging review step where users can exclude specific transactions |
| **Cluster ↔ Cluster merge** | Not implemented | `parentCluster` relationship for merging similar clusters. Currently no explicit merge workflow |

#### Proposed Cluster Lifecycle (target state)

![Proposed Cluster Lifecycle](images/MyFinance_ProposedClusterLifecycle.png)

**Two detection methods, one review flow (proposed target):**

Both paths to grow a cluster should go through a **`merging` review**, giving the user full control to **exclude specific transactions** before confirming cluster membership.
...
3. Creates **`merging` proposal**: linked to parent accepted cluster
4. User reviews: can **exclude** specific txns that don't truly belong
5. User **confirms** → remaining txns get `cluster_id` → merging deleted
